...
\begin{itemize}
    ...
    \item per $k=1 \quad \to \quad \mu_1$ è il valore di aspettazione o media (è una sorta di baricentro della distribuzione)
\end{itemize}
...
\begin{itemize}
    ...
    \item per $k=1 \quad \to \quad \bar{\mu}_1= \int_\Omega (x-\mu)f(x) dx = \int_\Omega x f(x) dx - \mu \underbrace{\int_\Omega f(x) dx}_{=1} = \mu - \mu = 0$
    \item per $k=2 \quad \to \quad \bar{\mu}_2=\sigma^2$
    \item per $k=3$ dove $\bar{\mu}_3=0$ è detto \textit{indice di asimmetria} o \textit{skewness}:
    \pgfplotsset{
        skewplot/.style={
    ...
    \item per $k=4 \quad \to \quad \bar{\mu}_4=\sigma^4$ è detto \textit{indice dicurtosi} o \textit{kurtosis}, valuta l'\textit{outlierness} della distribuzione, ovvero quando si pesano i valori dei valori estremi della distribuzione.
\end{itemize}
\vspace{1 cm}

Risulta utile introdurre per diversi motivi, per esempio:\\
Supponiamo di avere due variabili casuali $X$ e $Y$, supponiamo abbiano gli stessi momenti allora le distribuzioni ($f_X$ e $f_Y$) di probabilità sono identiche

\begin{definizione}
    \textbf{Funzione generatrice dei momenti}:
    \[
    M_x(t)=E[e^{tx}]=\int_\Omega e^{tx}f(x) dx \rightarrow \text{caso continuo}
    \]
    \[
    M_x(t)=\sum_i p_i e^{tx_i} \rightarrow \text{caso discreto}
    \]
\end{definizione}
Questa funzione permette di calcolare i momenti della distribuzione derivando la funzione rispetto a $t$ e ponendo $t=0$. Cioè:
\[
\left[ \frac{\partial ^m M_x}{\partial t^m} \right]_{t=0} =E[x^m] =\mu_m
\]
\textbf{Dimostrazione:}
Posso sfruttare la serie di Maclaurin:

\[ 
e^{tx} = \sum_{k=0}^{\infty} \frac{(tx)^k}{k!} 
\]
\[
M_x(t) = \int_\Omega e^{tx} f(x) dx = \int_\Omega \sum_{k=0}^{\infty} \frac{(tx)^k}{k!} f(x) dx = \sum_{k=0}^{\infty} \frac{t^k}{k!} \int_\Omega x^k f(x) dx = \sum_{k=0}^{\infty} \frac{t^k}{k!} \int_\Omega x^k f(x) dx = \sum_{k=0}^{\infty} \frac{t^k}{k!} \mu_k
\]
Esplicitando la somma: 
\[
M_x(t) = \mu_0 + \mu_1 t + \frac{\mu_2 t^2}{2!} + \ldots
\]

Se si calcola la derivata prima rispetto a t:
\[ \frac{\partial M_x}{\partial t} = \mu_1 + \frac{\mu_2 t}{1!} + \ldots \]
...
\begin{definizione}
    ...
    \[
    \bar{M}_x(t)=E[e^{t(x-\mu)}]=\int_\Omega e^{t(x-\mu)}f(x) dx \rightarrow \text{caso continuo}
    \]
    \[
    \bar{M}_x(t)=\sum_i p_i e^{t(x-\mu_i)} \rightarrow \text{caso discreto}
    \]
\end{definizione}
Nuovamente:
\[
...
Inoltrè è facile passare dalla funzione generatrice dei momenti a quella generatrice dei momenti centrali, infatti:
\[
\bar{M}_x(t)=E[e^{t(x-\mu)}]=E[e^{tx}e^{-t\mu}]=e^{-t\mu}E[e^{tx}]=e^{-t\mu}M_x(t)
\]

Infine se supponiamo di avere due variabili casuali $X$ e $Y$ dove queste hanno stesso $M_x(t)=M_y(t)$ o $\bar{M}_x(t)=\bar{M}_y(t)$ allora le distribuzioni ($f_X$ e $f_Y$) di probabilità sono identiche

\begin{definizione}
    \textbf{Funzione caratteristica}:
    \[
    \phi (x) = E[e^{itx}]=\int_\Omega e^{itx}f(x) dx \rightarrow \text{Trasformata di Fourier!}
    \]
\end{definizione}

Supponiamo di avere una variabile aleoria $Z=X+Y$, dove $X$ e $Y$ sono variabili aleatorie \textbf{indipendenti}, allora la distribuzione di probabilità di $Z$ è data da:
    \[
    P_Z=P_X \cdot P_Y
    \]
dunque la funzione generatrice dei momenti di $Z$ è data da:
\[
...
\[
\text{ripetendo $n$ volte l'esperimento, la probabilità di avere $k$ successi è data da $P(k;n,p)$}
\]
\[
P(k;n,p)=\binom{n}{k}p^k q^{n-k} = \frac{n!}{k!(n-k)!}p^k q^{n-k}=\frac{n!}{k!(n-k)!}p^k (1-p)^{n-k}
\] 
...
\end{itemize}
\begin{example}
    Supponiamo di avere 3($n$) oggetti ABC. 2 ($k$) oggetti sono stati scelti, posso prenderli in 3 modi: AB,BC,AC.

    Infatti:
    \[
    \frac{3!}{2!(3-2)!} = \frac{3!}{2! \cdot 1!} = \frac{6}{2 \cdot 1} = 3
    \]
\end{example}
...
La funzione generatrice dei momenti della distribuzione binomiale è data da:
\[
M_k(t)=E[e^{tk}]=\sum_{k=0}^{n} \underbrace{e^{tk} p^k}_{pe^{tk}} q^{n-k} \binom{n}{k}
\]
Infattiricordando che: $(a+b)^n = \sum_{k=0}^{n} \binom{n}{k} a^k b^{n-k}$, posso riscrivere la funzione generatrice dei momenti come:
\[
M_k(t)=(pe^t+q)^n
\]

\begin{itemize}
    \item Valore di aspettazione:
            \[
            M'_k(t)= n(pe^t+q)^{n-1}pe^t \rightarrow M'_k(0)=(n)(p+1-p)^{n-1}p=np=\mu
            \]
    \item Varianza:
            \[
            M''_k(t)= n(n-1)(pe^t+q)^{n-2}(pe^t)^2 \rightarrow M''_k(0)=n(n-1)p^2=np(1-p)=\sigma^2\]
            \[
            M''_k(0)=np(p+1-p)^{n-1}+n(n-1)(p+1-p)^{n-2}p=np+np^2(n-1)=E[x^2]
            \]
            dunque:
            \[
            var(x)=E[x^2]-E[x]^2=np+np^2(n-1)-n^2p^2=npq
            \]
    \item $\bar{\mu_3}=npq(1-2p)$ allora se: 
    \begin{itemize}
        \item $\bar{\mu_3}>0$ allora la distribuzione e' \textbf{asimmetrica} (skewed a destra)
        \item $\bar{\mu_3}=0$ allora la distribuzione e' \textbf{simmetrica}
        \item $\bar{\mu_3}<0$ allora la distribuzione e' \textbf{asimmetrica} (skewed a sinistra)
    \end{itemize}
\end{itemize}
