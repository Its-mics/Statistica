...
\usepackage{amsmath}
\usepackage{sectsty}
% Avoid mathabx redefining common symbols (e.g. \div)
\let\div\relax
\usepackage{mathabx}
\usepackage{pgfplots}
\usepackage{booktabs}
...
\author{ Mirolo Manuele / Alessio Brusini / Michele Tedeschini}
\date{a.a. 2025/26}
\newtheorem{theorem}{Teorema}


% --- Configurazione Fancyhdr  ---
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small \nouppercase{\leftmark}}
...
\newcommand{\lezione}[2]{
\newpage
\renewcommand{\thelezione}{#2} % Mantiene la data nell'intestazione della pagina
\section{#1} % Inserisce il titolo (#1) nell'indice automaticamente
...
\textbf{Osservazione:}
\begin{itemize}
    \item per $k=0 \to  \mu_0 =\int_\Omega f(x) dx  \mu_0=1$ (normalizzazione della funzione di probabilità)
    \item $\mu_1$ è il valore di aspettazione o media
\end{itemize}
...
\begin{itemize}
    \item $\bar{\mu}_0=1$ 
    \item $\bar{\mu}_1=0$
    \item $\bar{\mu}_2=\sigma^2$
    \item $\bar{\mu}_3=0$ è detto \textit{indice di asimmetria} o \textit{skewness}, per una distribuzione simmetrica (es. Gaussiana) vale 0;
    
    La distribuzione è asimmetrica a destra se $\bar{\mu}_3>0$ (es. Maxwell-Boltzmann) e asimmetrica a sinistra se $\bar{\mu}_3<0$.
    \item $\bar{\mu}_4=\sigma^4$ è detto \textit{indice dicurtosi} o \textit{kurtosis}, valuta l'\textit{outlierness} della distribuzione.
\end{itemize}

Supponiamo di avere due variabili casuali $X$ e $Y$, supponiamo abbaino gli stessi momenti allora le distribuzioni ($f_X$ e $f_Y$) di probabilità sono identiche
...
\[
[\frac{\partial ^m M_x}{\partial t^m}=E[x^m] =\mu_m]
\]
\textbf{Dimostrazione:} \[ 
f(x) = f(0)+f'(0)x+f''(0)x^2 \\
e^{tx} = 1 + tx + \frac{t^2 x^2}{2!} + \ldots \\
M_x(t) = \int_\Omega \sum_k \frac{(tx)^k}{k!}f(x) dx= \sum_k \frac{t^k}{k!} \int_\Omega x^k f(x) dx = \sum_k \frac{t^k}{k!} E[x^k] = \mu_0 + \mu_1 t + \frac{\mu_2 t^2}{2} + \ldots \\
\text{derivando}
\frac{\partial M_x}{\partial t} = \mu_1 + 2 \mu_2 t + \ldots \\
\text{e valutando in } t=0 \text{ ottengo } \mu_1
\]


\begin{definizione}
    \textbf{Funzione generatrice dei momenti centraali}:
    \[
...
Nuovamente:
\[
[\frac{\partial ^m \bar{M}_x}{\partial t^m}=E[(x-\mu)^m] =\bar{\mu}_m]
\]
...
\[
\sigma^2=E\left[ g(x)-E(g(x))\right]^2
\]
svolgendo i calcoli:
\[
