...
Quando analizziamo un set di dati sperimentali $N \to \{x_1, x_2, \dots, x_N\}$, possiamo stimare la distribuzione di probabilità sottostante tramite un istogramma. L'istogramma divide il range dei dati in intervalli (bin).

Se la variabile $x$ è descritta da una densità di probabilità $P(x)$, la probabilità $p_i$ che un evento cada nel bin $i$-esimo (di larghezza $\Delta x$) è:
\[
p_i = \int_{x_i}^{x_i+\Delta x} P(x) dx \simeq P(\bar{x}_i)\Delta x
\]
dove $\bar{x}_i$ è il valore medio del bin \textit{i}.
Se indichiamo con $n_i$ il numero di eventi conteggiati nel bin $i$, questa è una variabile aleatoria che segue una distribuzione Binomiale (o Poissoniana se $N$ è grande e $p_i$ piccolo):
\[
n_i \sim \text{Binomiale}(N, p_i)
\]
Di conseguenza, il valore atteso per l'altezza dell'istogramma nel bin $i$ è:
\[ 
E[n_i] = N p_i \simeq N \cdot P(\bar{x}_i) \Delta x 
\]
...
Questo ci permette di confrontare i dati sperimentali con la curva teorica $P(x)$. Devo tenere conto che per confrontare le due devo sempre riscalare la mia funzione 
di probabilità $P(x)$, con il fattore $\Delta(x)N$.\\
Ricordiamo che la varianza degli eventi che cado in un determinato intervallo è dato da $\sqrt{n_i}$.



\lezione{Funzione di distribuzione di Gauss}{02 Febbraio 2026}
\subsection{Parte di Bressan da integrare dopo}
...
\subsubsection{Esempio di distirbuzione di Gauss}
...
\subsubsection{Dimostrazione Euriatica}
...
La distribuzione normale (dal fatto che le variabili naturali seguivano secondo Gauss questa distribuzione) è rappresentata dalla formula:
\[
f(x; \mu , \sigma ) = N(\mu,\sigma^2)=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\mu)^2}{2\sigma^2}}
\]
dove il simbolo $N$, indica il fatto che è una distribuzione normale. 
Se voglio normalizzara posso fare il cambio di varibile $z=\frac{x-\mu}{\sigma}$, notando che vale l'identità:
\[
f_z[z]dz=f_x[x]dx= f_x[x(z)]\frac{dx}{dz}dz= \frac{1}{\sqrt{2 \pi}}e^{\frac{-z^2}{2}}dz= N(0,1)
\]
La funzione genratrice di momenti medi è data da:
\[
\bar{M}_x[t]=\frac{1}{\sqrt{2 \pi \sigma^2}}\int_{-\infty}^{\infty}\exp(t(x-\mu)- \frac{{(x-\mu)^2}}{2 \sigma^2})dx =  \frac{1}{\sqrt{2 \pi \sigma^2}}\int_{-\infty}^{\infty}\exp(-\frac{{(x-\mu- \sigma^2 t)}^2}{2 \sigma^2})\exp(\frac{\sigma^2 t^2}{2})dx \]
\[ \boxed{\bar{M}_x[t]=\exp(\frac{\sigma^2 t^2}{2})}
\]
valendo inoltre la formula nota $M_x[t]= e^{\mu t} \bar{M}_x[t]$, ottengo:
\[
\boxed{M_x[t]= \exp(\mu t + \frac{\sigma^2 t^2}{2})}
\]
Nelle formule sopra ho utilizzato il fatto che:
\[
t(x- \mu) - \frac{{(x-\mu)}^2}{2 \sigma^2}= -\frac{{(x-\mu-\sigma^2t)}^2}{2\sigma^2} + \frac{\sigma^2 t^2}{2}
\]
...
Avendo una variabile del tipo: 
\[
S= \sum_{i=1}^{n}\alpha_i x_i
\]
dove $x_i \sim N(\mu_i,{\sigma_i}^2)$ e $\alpha_i$ sono i coefficienti. Usando ora le proprietà delle funzioni generatrici di momento e chiamando $y_i= \alpha_i x_i$ ho che:
\[
M_S[t]= \prod_{i=1}^{n} M_{y_i}[t]= \prod_{i=1}^{n} M_{x_i}[\alpha_i t]= \prod_{i=1}^{n} \exp(\mu_i \alpha_i t + \frac{{\sigma_i}^2{\alpha_i}^2 t^2}{2})=\exp((\sum_{i=1}^{n}\alpha_i \mu_i)t   + \frac{1}{2}(\sum_{i=1}^{n}{\sigma_i}^2{\alpha_i}^2)t^2)
\]
\[
M_S[t]= \exp(\tilde{\mu}t + \frac{1}{2}{\tilde{\sigma}}^2 t^2)
\]
