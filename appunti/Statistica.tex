...
\lezione{Distribuzione di probabilità a più variabili}{23/03/2026}
Consideriamo due variabili alleatori X,Y. La loro distribuzione di probabilità congiunta è data da: \[P_{X,Y}(x,y) = \text{Prob}(x<X<x+dx, y<Y<y+dy)\] che rappresenta la probabilità che $X$ assuma il valore $x$ e $Y$ assuma il valore $y$ contemporaneamente. 
La distribuzione congiunta è normalizzata, cioè $\int \int P_{X,Y}(x,y) dx dy = 1$. \\
La funzione di ripartizione congiunta è definita da:
\[
\int_{-\infty}^{x}du \int_{-\infty}^{y}dv P_{X,Y}(u,v) = F(x,y) 
\]
La \textit{probabilità marginale} di $X$ è data da:
\[
\int_{-\infty}^{+\infty} P_{X,Y}(x,y) dy = P_X(x)
\]
Stesso concetto per $Y$. La \textit{probabilità condizionata} di $X$ dato da $Y$ è data da:
\[
\Pi (x | y) = \frac{P_{X,Y}(x,y)}{P_Y(y)}
\]
Se $X$ e $Y$ sono indipendenti, allora $P_{X,Y}(x,y) = P_X(x) P_Y(y)$ e quindi $\Pi (x | y) = P_X(x)$, cioè la probabilità di $X$ non dipende da $Y$. \\
Per ricavare la probabilità congiunta $P_{X,Y}(x,y)$ è scrivibile come: $ P_{X,Y}(x,y)= P_Y(y) \Pi (x|y)$. \\
Prendendo dunque una qualsiasi funzione $f(x,y)$, il suo valore di aspettazione è dato da:
\[
E[f(x,y)]= \int_{-\infty}^{\infty}du \int_{-\infty}^{\infty}dv P_{X,Y}(u,v) f(u,v)
\]
Dunque i miei momenti sono dati da: 
\[
\mu_{m,n}= E[x^n y^m]
\]
dove valgono le relazioni: $\mu_{0,0}=1$, $\mu_{1,0}=\mu_x$, $\mu_{0,1}=\mu_y$\\
I momenti centrali, rispetto al $\mu_x$ e $\mu_y$ sono dati da: 
\[
\bar{\mu}_{n,m}= E[{(x-\mu_x)}^n {(y-\mu_y)}^m]
\]
Dunque ho le seguente proprietà: $\bar{\mu}_{2,0}= \text{var}(x)$, $\bar{\mu}_{1,1}= \text{cov}(x,y)$.\\

Consideriamo ora il caso di due variabili discrete $X$ e $Y$; il valore di aspettazione di una determinata funzione è dato da:
\[
E[f(x,y)]= \sum_{i,j} P_{X,Y}(x_i,y_j) f(x_i,y_j)
\]
dunque la covarianza è descrivibile da:
\[
\text{cov}(x,y)= E[(x-\mu_x)(y-\mu_y)]= \sum_{i,j} P_{X,Y}(x_i,y_j) (x_i-\mu_x)(y_j-\mu_y) = E[xy] - \mu_x \mu_y
\]
Da qui è evidente che se $X$ e $Y$ sono indipendenti, allora $E[xy] = \sum_{i,j}^{} P_{X,Y}(x_i,y_j) x_i y_j = E[x] E[y]$, per cui $\text{cov}(x,y)=0$. 

\begin{example}
    Supponiamo la funzione $z= ax+ by$, con ipotesi che $E[x]=E[y]=0$, allora $E[z]=0$.\\ Possiamo calcolare allora la varianza di $z$:
    \[
    \text{var}(z)= E[{(z-E[z])}^2] = E[z^2]= a^2 E[x^2] + b^2 E[y^2] + 2abE[xy]= a^2 \text{var}(x) + b^2 \text{var}(y) + 2ab \text{cov}(x,y)
    \]
    Se invece avessi avuto due variabili, con valore di aspettazione non nulla, $\tilde{x}= x - E[x]$ e $\tilde{y}= y- E[y]$, allora avrei avuto:
    \[
    \text{var}(\tilde{x})= \text{var}(x), \quad \text{var}(\tilde{y})= \text{var}(y), \quad \text{cov}(\tilde{x},\tilde{y})= \text{cov}(x,y)
    \] 
\end{example}
Utile è introdurre la matrice delle covarianze, che è una matrice simmetrica definita da:
\begin{equation}
    \Sigma = \begin{pmatrix}
    ...
    \end{pmatrix}
\end{equation}
Questa matrice nel caso di variabili indipendenti è diagonale, mentre nel caso di funzioni lineari mi resistuisce la varianza delle mia variabile z.\\
Infatti nel nostro esempio precedente, avendo $z=ax+ by$, posso definire il vettore $v= (a,b)$, per ottenere che:
\[
\text{var}(z) = v^T \Sigma v
\]

Il \textit{coefficiente di correlazione} è definito come:
\[
\rho = \frac{\text{cov}(x,y)}{\sqrt{\text{var}(x) \text{var}(y)}} \quad -1 \leq \rho \leq 1
\]
È una misura adimensionale che indica la forza e la direzione della relazione lineare tra due variabili. 
Per dimostrarlo uso la variabile $z = \sigma_y x \pm \sigma_x y$, dunque ottengo che:
\[
\text{var}(z) = \sigma_y^2 \text{var}(x) + \sigma_x^2 \text{var}(y) \pm 2\sigma_x \sigma_y \text{cov}(x,y) = 2\sigma_x^2 \sigma_y^2 (1 \pm \rho) \geq 0
\]
Dunque essendo i termini quadratici sempre negativi trovo la relazione $1 \pm \rho \geq 0$. 
\begin{example}
    Consideriamo $y= mx +q$, allora posso calcolare il mio coefficiente di correlazione:
    \begin{itemize}
        \item $E[y] = mE[x] + q$ 
        \item $\text{var}(y) = m^2 \text{var} (x)$
        \item $E[xy] = m E[x^2] + q E[x]$
        \item $\text{cov}(x,y) = E[xy] - E[x]E[y] = m \text{var} (x)$
        \item $\rho = \frac{m}{|m|} = \pm 1$
    \end{itemize}
\end{example}

\subsection{Multinormale o Gaussiana Multivariata}
La distribuzione multinormale è una generalizzazione della distribuzione normale a più variabili. Noi studieremo principalmente il caso di due variabili $X$ e $Y$.\\
Nel caso di variabili indipendetni questa è rappresentata da: 
\[
P_{X,Y}(x,y)= \frac{1}{2 \pi \sigma_x \sigma_y } \exp(-\frac{1}{2}[\frac{{(x-\mu_x)}^2}{{\sigma_x}^2} + \frac{{(y-\mu_y)}^2}{{\sigma_y}^2}]) = \frac{1}{2 \pi \sigma_x \sigma_y} e^{-\frac{Q^2}{2}}
\]
dove Q è il mio termine in parentesi sotto radice. 


\subsection{\color{red} Funzione di distribuzione per variabili casuali}
