...
\begin{theorem}
    ...
    \]
    allora per $n \to \infty$ $S \approx \mathcal{N} (\sum_{i=1}^{n} x_i,\sum_{i=1}^{n} \sigma_i^2)$
\end{theorem}
Nella pratica basta $n=10$. 

Posso anche scrivere, svolgendo il cambio di variabili $z=\frac{x-\mu}{\sigma}$, ho la variabile aletoria:
\[
\frac{S-\sum_{i=1}^{n} \mu_i}{\sqrt{\sum_{i=1}_{n} \sigma_i}} \approx \mathcal{N}(0,1)
\]
...
\[
\frac{S-n\mu}{\sigma \sqrt{n}} = \frac{S-\frac{\sum_{i=1}^{n} \x_i}{n}}{\frac{\sqrt{n}}{n} \sigma} \approx \mathcal{N}(0,1)
\]
dove ho introdotto la media campionaria:
\[
\bar{x}=\frac{\sum{i=1}{n}x_i}{n}
\]
...
\[
\lim_{n \to \infty} \frac{S-n\mu}{\sigma \sqrt{n}} \approx mathcal{N}(0,1)
\]
...
\]
infatti per ogni variabile definita come $v=x-c \text{c costante}$, ottengo:
\[
E[v]=E[x]-c
...
\[
M_y(\frac{t}{\sqrt{n}})=E[e^{\frac{t}{\sqrt{n}}y}] \approxeq 1+ \underbrace{\frac{t}{\sqrt{n}}E[y]}_{=0}+\frac{t^2}{2n}E[y^2]+ \cdots = 1+\frac{t^2}{2n}+ \cdots 
\]
notando che:
\[
...
\[
M_Z(t)=\left(1+\frac{t^2}{2n}\right) \rightarrow (n \to \infty) 1+\frac{t^2}{2}+ \cdots = e^{\frac{t^2}{2}} 
\]
che è la funzione generatrice di momenti della distribuzione normale standard, dunque $Z \approx \mathcal{N}(0,1)$ e con un cambio di variabile ottengo il risultato voluto.

Infine:
\[
Z=\frac{\sum_{i=1}^{n}}{\sqrt{n}}=\frac{1}{\sqrt{n}}\sum_{i=1}^{n}=\left(\sum{i=1}^{n}\frac{x_i-\mu}{\sigma}\right) = \frac{\sum_{i=1}^{n}x_i-\mu}{\sqrt{n}\sigma} \approx \mathcal{N}(0,1)
\]
q.e.d.
...
Supponiamo di avere una serie di variabili con distribuzione uniforme:
\[
P(u)=
\begin{cases}
    1 & u \in [0,1]
...
\text{var}(x)=E[u]-(E[u])^2=\frac{1}{12}
\]
La variabile standardizzata sarà:
\[
\frac{\sum_i u_i-nE[u]}{\sqrt{n\text{var}(u)}}=\frac{\sum_i u_i-\frac{n}{2}}{\sqrt{\frac{n}{12}}} \approx \mathcal{N}(0,1)
\]
molto utilizzata nel caso $n=12$, poichè in tal caso:
\[
\sum_i u_i-6\approx \mathcal{N}(0,1)
\]
dunque per simulare la parte centrale della gaussiana posso utilizzare questa approssimazione, prendo 12 variabili uniformi e gli sottraggo 6.
...
Ma prima, un piccolo compendio: supponiamo di avere una variabile $y=f(x)$ allora la distribuzione sarà $P_y(y)=P_x[x(y)]\mod \frac{dx}{dy}$

Nel caso di più variabili:
$P_y(y_1, ..., y_n)=P_x(x_1,..., x_n)\mod J
$
dove $\mod J$ è il determinante della matrice Jacobiana.

Supponendo di avere le variabili $X_1,X_2 \apprx u(0,1)$, posso definire le variabili:
\begin{itemize}
    \item $Y_1=\sqrt{-2 2n X_1} \cos(2\pi X_1)$
    \item $Y_2=\sqrt{-2 2n X_2} \cos(2\pi X_2)$
\end{itemize}
...
con determinante: 
\[
\frac{\partial X_1}{\partial Y_1} \frac{\partial X_2}{\partial Y_2} - \frac{\partial X_1}{\partial Y_2}\frac{\partial X_2}{\partial Y_1}
\]
dove:
\[
...
e trovo:
\[
\text{det(J)}=\frac{1}{2\pi} \exp (-\frac{1}{2}(Y_1^2+Y_2^2)) \frac{1}{Y_1^2+Y_2^2}(Y_1^2+Y_2^2) = \frac{1}{2\pi} \exp (-\frac{1}{2}(Y_1^2+Y_2^2)) 
\]
allora:
\[
ss
