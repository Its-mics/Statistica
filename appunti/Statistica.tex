...
vediamo ora come si ricava la   distribuzione esponenziale da quella di poisson. \\ 
eventi aleatori -> numero medio v
\[
P(x, \lambda) = \frac{v^x e^{-v}}{x!} 
\]
tempo t
$\lambda \to$ numero medio di eventi in un intervallo di tempo t
$v  = \lambda t$

\[
P(k, \lambda t) = \frac{(\lambda t)^k e^{-\lambda t}}{k!} \quad \text{con } k = 0, 1, 2, \dots
\]
T = tempo attesa tra eventi successivi


\[
Prob (T> t) = Prob (0, \lambda t) = e^{-\lambda t}    
\]


\[
Prob (T< t) = 1- Prob (T> t) = 1-e^{-\lambda t}
\]

Funzione di ripartizione x' variabile aleatoria
\[
Prob(x' <= x) = F_{x'}(x) = \int_{-\infty}^{x} P_{x'}(z) dz
\]

\[
x \to \infty \quad \int_{-\infty}^{\infty} P_{x'}(x) dx = 1 
\]
 

% --- 1. Grafico della Funzione di Ripartizione F(x) ---
\begin{tikzpicture}
\begin{axis}[
    axis lines = left,
    xlabel = {$x$},
    ...
    samples=100,
    grid=major,
    title={Funzione di Ripartizione $F(x) = 1 - e^{-\lambda x}$}
]
    \addplot [very thick, blue] {1 - exp(-1*x)};
    \draw [dashed] (axis cs:0,1) -- (axis cs:5,1) node[right] {$1$};
    \node at (axis cs:2,0.5) [anchor=south] {$\lambda = 1$};
\end{axis}
\end{tikzpicture}

% --- 2. Grafico della Densità di Probabilità P(t) ---
\begin{tikzpicture}
\begin{axis}[
    axis lines = left,
    xlabel = {$t$},
    ...
    domain=0:5,
    samples=100,
    title={Densità di Probabilità $P(t) = \lambda e^{-\lambda t}$}
]
    \addplot [very thick, red] {1 * exp(-1*x)};
    \node at (axis cs:0,1) [anchor=east] {$\lambda$};
    \node at (axis cs:2,0.4) [anchor=south] {Decadimento esponenziale};
\end{axis}
...
\]
 ...
 il risultato vuol dire che itempi di attesa piccoli sono più grandi di quelli grandi
% --- 3. Grafico dell'area sottesa (Integrale tra t1 e t2) ---
\begin{tikzpicture}
\begin{axis}[
    axis lines = left,
    xlabel = {$t$},
    ...
    domain=0:5,
    samples=100,
    title={Probabilità come area tra $t_1$ e $t_2$}
]
    % Curva principale
    \addplot [very thick, black] {1 * exp(-1*x)};
    
    % Area ombreggiata
    \addplot [fill=orange, fill opacity=0.3, draw=none, domain=0.8:2.5, samples=50] {1*exp(-1*x)} \closedcycle;
    
    % Etichette t1 e t2
    \draw [dashed] (axis cs:0.8, 0) -- (axis cs:0.8, 0.449) node[pos=0, below] {$t_1$};
    \draw [dashed] (axis cs:2.5, 0) -- (axis cs:2.5, 0.082) node[pos=0, below] {$t_2$};
    
    \node at (axis cs:1.6, 0.15) {Area $= \int_{t_1}^{t_2} \lambda e^{-\lambda t} dt$};
\end{axis}
\end{tikzpicture}
...
\]
che graficamente è l'area sottesa alla curva tra $t_1$ e $t_2$ 

dove $\lambda$ è il numero medio per unità di tempo e $\tau = \frac{1}{\lambda}$ 
è il tempo medio di attesa quindi:
 
\[ 
\lambda e^{-\lambda t} \to \frac{1}{\tau} e^{\frac{-t}{\tau}}
\]

Assenza di memeoria
eventi A,M prob A condizionata e M


\[
Prob(A|M) = \frac{Prob(A \cap M)}{Prob(M)}
\]

ho fià asepttato $\to s, prob t$
...
\[
Prob(T > s+t | T > s) = \frac{Prob(T > s+t)}{Prob(T > s)} = \frac{e^{-\lambda (s+t)}}{e^{-\lambda s}} = e^{-\lambda t} = Prob(T > t)
\]


\subsection{Distribuzione di Erlang}
tempo di attesa del k-esimo evento  ($T_k$)


$T_1 \sim P_1(t,\lambda) = \lambda e^{-\lambda t}$
voglio capire come è distribuito $T_2$.
Prendo un intervallo t in cui suppongo ci siano 2 eventi indico con x il tempo di attesa del primo quindi quello del secondo sarà $t-x$


\begin{tikzpicture}[node distance=2cm, font=\small]

    % Asse temporale
    \draw[thick, ->] (0,0) -- (8,0) node[right] {tempo};

    % Punti significativi (pallini neri)
    \filldraw (0,0) circle (2pt) node[below=5pt] {$0$};
    \filldraw (4,0) circle (2pt) node[below=5pt] {$x$};
    \filldraw (7,0) circle (2pt) node[below=5pt] {$t$};

    % Graffe superiori (Versione standard "brace")
    \draw [decorate, decoration={brace, amplitude=8pt, raise=5pt}, thick]
      (0,0) -- (4,0) node [midway, above=15pt] {$T_1 \sim \text{Exp}(\lambda)$};
      
    \draw [decorate, decoration={brace, amplitude=8pt, raise=5pt}, thick]
      (4,0) -- (7,0) node [midway, above=15pt] {$T_2 - T_1 \sim \text{Exp}(\lambda)$};

    % Frecce per gli intervalli sotto (x e t-x)
    \draw[<->, >=stealth, blue!70!black] (0,-0.8) -- (3.9,-0.8) node[midway, fill=white] {$x$};
    \draw[<->, >=stealth, red!70!black] (4.1,-0.8) -- (7,-0.8) node[midway, fill=white] {$t-x$};

    % Graffa totale sotto
    \draw [decorate, decoration={brace, mirror, amplitude=10pt, raise=35pt}]
      (0,0) -- (7,0) node [midway, below=50pt] {Tempo totale $T_2 = t$};

\end{tikzpicture}

$T_2 \sim P_2 (t, \lambda) = \int_0^t P_1 (x,\lambda) P_1 ((t-x), \lambda) dx  = \int_0^t \lambda e^{-\lambda x} \cdot \lambda e^{-\lambda (t-x)} dx = \lambda^2 e^{-\lambda t} \int_0^t e^{\lambda x} e^{-\lambda t} dx = \lambda^2 e^{-\lambda t} \int_0^t e^{-\lambda (t-2x)} dx$
$= \lambda^2 t e^{-\lambda t}$

\[
P_3 (t, \lambda) = \int_0^t P_2 (x, \lambda) P_1 ((t-x), \lambda) dx = \int_0^t \lambda^2 x e^{-\lambda x} \cdot \lambda e^{-\lambda (t-x)} dx = \lambda^3 e^{-\lambda t} \int_0^t x e^{\lambda x} e^{-\lambda t} dx = \lambda^3 e^{-\lambda t} \int_0^t x e^{-\lambda (t-2x)} dx
\]
\[
\lambda^3 \frac{t^2}{2} e^{-\lambda t}
\]


Erlang
\[
P_k (t, \lambda) = \lambda^k \frac{t^{k-1}}{(k-1)!} e^{-\lambda t}
\]


il valore di aspettazione è $E[T_k] = \frac{k}{\lambda}$ e la varianza è $\text{Var}(T_k) = \frac{k}{\lambda^2}$
...
$x \sim f(x)$
metodo di accept-reject: 
\begin{itemize}
    \item prendo un intervallo $[a,b]$ in cui $f(x)$ è definita e prendo un punto $y$ che sia più grande del massimo di $f(x)$ in quell'intervallo
    \item estraggo un numero casuale $x_0$ uniformemente distribuito in $[a,b]$ e un numero casuale $y_0$ uniformemente distribuito in $[0,y]$
    \item se $y_0 < f(x_0)$ allora accetto $x_0$ altrimenti lo rifiuto
\end{itemize}

Metodo della funzione di ripartizione:\
\[
F(x) = Prob (x'<= x) = \int_{-\infty}^{x} P(z) dz
\]

visto che $0<= F(x) <= 1$
$u\sim U(0,1) \to u=F(x) \to x=F^{-1}(u)$

\[
Prob(F^{-1}<= x) = Prob(u' <= F(x)) = F(x)
\]
\[
Prob(u'<= y) = \int_{0}^{y} dx = y
\]
